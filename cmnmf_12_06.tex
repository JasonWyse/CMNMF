...
Since the known gene-phenotype associations are sparse and only cover a part of genes and phenotypes, the regularization with $L_2$ norm is proposed to control the sparseness of $G$, $P_1$ and $P_2$. The coefficients ${\lambda_1}$ and $\lambda_2$ balance the regularization terms. Finally, the framework of regularized CMNMF with sparse constraint is to minimize the loss function formulated as follows:
...
\subsection*{\textbf{Optimization Framework}}
...
If we fix variables $P_1$ and $P_2$,
...
%\end{split}
%\end{equation}
using the Karush-Kuhn-Tucher(KKT) conditions $\phi_{ij}G_{ij}=0$, the partial derivative of $L$ with respect to $G$ is:
...
\end{equation}
To satisfy the equality constraint, we normalize $G_{ij}$ in rows.
%\begin{equation}\label{equ:updating_G}\nonumber
...
%\end{split}
%\end{equation}
the partial derivative of $L$ with respect to $P_1$ and $P_2$ are:
...
\end{split}
\end{equation}
To satisfy the equality constraint, we normalize $(P_1)_{ij}$ and $(P_2)_{ij}$ in columns.
%as
...
%\end{split}
%\end{equation}
the partial derivative of $L_s$ with respect to $G$ is:
\begin{equation}\label{equ:G_gradient}\nonumber
\begin{split}
\frac{\partial{L_s}}{\partial{G}}=
...
\hline
\multirow{3}{*}{204}&
\multirow{3}{11em}{GCK, PDX1, INS2,\\E2F1, HNF1A,TNF, \\LEP, RPS6KA3,\\AKT2, SLC5A1, et.al}&
...
\hline
\multirow{3}{*}{138}&
\multirow{3}{11em}{GCK, SLC2A4, IRS1,\\INS2, IRS2, LDLR, LIPA,\\PCSK1, POMC,et.al}&
