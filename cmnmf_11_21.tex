...
%\end{split}
%\end{equation}
An other measurement on gene ontology for prediction clustering result $Y=\{Y_1,...,Y_s\}$, we use $(Sim)_{between}$ to denote the average similarity between different gene clusters,$(Sim)_{between}$ to denote average similarity of different genes within gene clusters. then we defined R as:
$$
R = \frac{(Sim)_{between}}{(Sim)_{within}}
$$
where 
\begin{equation}\label{}\nonumber
\begin{split}
...
\end{equation}
,
$P=\{(i,j)|1\leq i< j\leq|Y|\}$, $FS_{simGIC}(G_k,G_l)$\cite{} is used to measure the similarity between gene $G_k$ and $G_l$ by GO terms.
%\begin{equation}\label{}\nonumber
%Jaccard\ Index=\frac{TP}{TP+FP+FN}
%\end{equation}
%\begin{equation}\label{}\nonumber
%Rand\ Index=\frac{TP+TN}{TP+FP+FN+TN}
%\end{equation}
%when $\beta$ is bigger, R takes more portion in $F_{\beta}$, when $\beta=1$, it's the well-known $F_1$ measure.

The results are shown in Table().
%\begin{table}[!h]
%\centering
%\caption{F-Score\ jaccard-Score\ GO\ computed by the cluster result under different clustering function.}\label{tab:measurements}
%\label{tab:go}
...
The clustering results are column-normalized by using z-score, and the $G_{ij}$ will be set as 0 if it is less than $3\sigma$. All the methods are repeated twenty times with different initial values and average performance results are reported in Fig.\ref{tab:F1}. 
