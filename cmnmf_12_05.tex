...
\textbf{Conclusions:}
The performance of our proposed method and other baselines (including HAC, Kmeans, Kernel-Kmeans, LDA, NMF,HMF) are evaluated on $F_1$ , $Jaccard\ Index$, $Rand\ Index$ and Gene Ontology similarity measure. The results show the effectiveness of our proposed method compared with other baselines. Additionally, we conduct the supervised Consistent Multiple Nonnegative Matrix Factorization(S-CMNMF) to predict gene-phenotype associations. S-CMNMF beats other baselines as well.
...
\section*{Method}
Fig.\ref{fig:model} illustrates the framework of the Consistent Multiple Non-negative Matrix
...
\begin{tabular}{|l|l|}
    ...
    $k_1$ & Number of phenotype ontology in parent level\\
    $k_2$ & Number of phenotype ontology in child level\\
    \hline
  \end{tabular}
\end{table}
...
We denote $\bm{A}_{(m \times n)}$ a binary gene-phenotype association matrix by $m$ genes and $n$ phenotypes, where $\bm{A}_{ij}$ is set with 1 for known association and 0 otherwise. The goal of factorizing matrix $\bm{A}$ based on NMF\cite{DanielD.Lee2001} is to derive gene functional clusters . The loss of it can be defined as:
\begin{equation}\label{equ:NMF}
\mathop{min}_{{\bm{P},\bm{G}}} \|\bm{A}-\bm{GP}\|^{2}_{F}
\end{equation}
Where $\bm{G}$ and $\bm{P}$ denotes gene clusters and phenotype clusters , respectively. The notation $||\bullet||_F$ means the $Frobenius$ norm of a matrix.
...
Motivated by above consistent assumption, we extract phenotype ontologies from two adjacent levels, and two gene-phenotype association matrices $\bm{A}_{1}$ and $\bm{A}_{2}$ are extracted from gene-phenotype text file. We assume that the factorizations on  $\bm{A}_{1}$ and $\bm{A}_{2}$ for the gene cluster should be consistent, although the genes are annotated by adjacent level phenotype ontologies. In our work, we use a common basis gene cluster matrix $\bm{G}$ to achieve this goal. The representation of the data can be derived by optimizing the following quadratic objective function:
\begin{equation}
...
If we fix variables $\bm{P}_1$ and $\bm{P}_2$,
...
By considering prior gene cluster information, our proposed CMNMF can be utilized for another task ,gene-phenotype association prediction problem, as well. We call it Supervised CMNMF(S-CMNMF) as including gene information prior. Thus, the loss function of S-CMNMF can be rewrite as:
...
\end{split}
\end{equation}
Using the Karush-Kuhn-Tucher(KKT) conditions $\bm{\Phi}_{ij}\bm{G}_{ij}=0$, the multiplicative update rule is:
\begin{equation}\label{equ:updating_G}\nonumber
\bm{G}_{ij}\leftarrow \bm{G}_{ij}
...
\hline
K-Means&0.0597  & 0.0307&0.9678& 0.7932\\
\hline
...
\hline
\multirow{3}{*}{204}&
\multirow{3}{11em}{GCK, PDX1, INS2,\\IRS2,E2F1, HNF1A, \\LEP, RPS6KA3,\\AKT2, SLC5A1, et.al}&
